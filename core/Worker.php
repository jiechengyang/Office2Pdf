...
class Worker
{
    ...
    {
        // 确定操作系统环境并确定php运行环境是否是cli模式
        static::checkSapiEnv();
        // 1、自定义的错误处理函数
        // 2、回溯跟踪
        // 3、初始化pid、log存储位置
        // 4、初始化状态为启动状态
        // 5、初始化进程数组【默认生成一个，count数大于1则重新生成count个】
        static::init();
        // 对当前启动服务的文件加锁
        static::lock();
        // 解析命令【eg:php server.php start | php server.php start -d】，windows不进行解析
        static::parseCommand();
        // 仅linux执行
        static::daemonize();
        // 仅linux执行
    ...
    protected static function parseCommand()
    {
        ...
        global $argv;// 传递给脚本的参数数组, 包含当运行于命令行下时传递给当前脚本的参数的数组
        // Check argv;
        $start_file = $argv[0];
        ...
        $master_is_alive = $master_pid && posix_kill($master_pid, 0) && posix_getpid() != $master_pid;// 向进程发送信号
        // Master is still alive? 主进程是否存在
        if ($master_is_alive) {
            if ($command === 'start') {
    ...
    {
        $files = static::getStartFilesForWindows();// 获取启动文件
        global $argv;
    ...
    public static function safeEcho($msg, $decorated = false)
    {
        ...
        fwrite($stream, $msg);// 将内容写入到流
        fflush($stream);//  将缓冲内容输出到文件
        return true;
    }
    ...
    {
        if (!$stream) {// 获取标准输出流【键盘键入输出到屏幕，行缓冲，缓冲慢，直接i/o操作】
        ...
        if (($stat['mode'] & 0170000) === 0100000) {
            ...
            static::$_outputDecorated =
                ...
                posix_isatty($stream);// 确定文件描述符是否为交互式终端;确定文件描述符fd是否引用有效的终端类型设备
        }
    ...
    public function __construct($socket_name = '', $context_option = array())
    {
        ...
        $this->workerId                    = spl_object_hash($this);// 为指定对象返回一个唯一标识符。这个标识符可用于作为保存对象或区分不同对象的hash key
    ...
    public function listen()
    {
        ...
        if (!$this->_mainSocket) {
            ...
            if ($this->transport === 'ssl') {
                stream_socket_enable_crypto($this->_mainSocket, false);// 在已连接的套接字上打开/关闭加密
            } elseif ($this->transport === 'unix') {
                $socketFile = substr($address, 2);
                if ($this->user) {
                    chown($socketFile, $this->user);//  改变文件的所有者
                }
                if ($this->group) {
                    chgrp($socketFile, $this->group);// 改变文件所属的组
                }
            }
...
//            尝试打开TCP的keepalive并禁用Nagle算法
            if (function_exists('socket_import_stream') && static::$_builtinTransports[$this->transport] === 'tcp') {
                set_error_handler(function(){});
                $socket = socket_import_stream($this->_mainSocket);// 将封装套接字的流导入套接字扩展资源, 此函数好像不支持SSL连接
                // 设置套接字的套接字选项
                //    +使用Socket_Create()或Socket_Accept()创建的有效套接字资源。
                //    +level参数指定选项所在的协议级别。例如，要检索套接字级别的选项，将使用SOL_Socket的Level参数。其他级别(例如TCP)可以通过指定该级别的协议号来使用。可以使用getProtobyname()函数找到协议号。
                //    +可用的套接字选项与socket_get_option()函数的套接字选项相同。
                //    +选项值
                socket_set_option($socket, SOL_SOCKET, SO_KEEPALIVE, 1);
                socket_set_option($socket, SOL_TCP, TCP_NODELAY, 1);
                restore_error_handler();// 还原之前的错误处理函数 在使用 set_error_handler() 改变错误处理函数之后，此函数可以 用于还原之前的错误处理程序(可以是内置的或者也可以是用户所定义的函数)。
            }

            // Non blocking.
            stream_set_blocking($this->_mainSocket, 0);// 为资源流设置阻塞或者阻塞模式
        }
